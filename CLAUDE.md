...
**There are many documents, and one *rendered* one per browsing context** (ADR-0017, ADR-0035). `new Document()` / `DOMParser` / `createHTMLDocument` create real Document nodes. They are inert **structurally**, not defensively: `propagate_connectedness` grants `NodeFlags::IS_CONNECTED` only to a member of `DomTree::rendered_roots`, and style, layout, resource loading, custom-element upgrades, the `getElementById` index and event bubbling to the `Window` all gate on that flag. An `<iframe>`'s document joins that set through `add_rendered_root`; a `DOMParser` document never does, which is why it is still inert. So `IS_CONNECTED` means "in *a* rendered document" — and every consumer of it is a **routing** question, not a boolean: *which* engine, *which* frame. `DomTree::in_rendered_document` is the image-loading gate ADR-0028 D3 used to spell as `node_document(el) == dom.document()`; the *spec's* `isConnected` is `DomTree::is_spec_connected`, and only the JS getter uses it. A document member that reflects the browsing context (`defaultView`, `currentScript`, `styleSheets`, `readyState`, anything that flushes layout) must check `this == cx.state.frame.document()`, **not** `dom.document()` — `imp/document.rs::is_page_document` is exactly that, and getting it wrong is how a script in an iframe silently mutates the page's tree. Reaching a node's *engines* from another realm (`iframe.contentDocument.body.offsetWidth`) goes through `BindCx::frame_for`. **"Which document" is `containing_document`, never `node_document`**: a node inside a shadow tree is owned by its shadow root, so `node_document` can answer with a `DocumentFragment`. **A `document::` member takes a *document*** — `scrollParent` once passed an Element and got away with it only because the parameter was ignored.
...
**Reflow can fail, and a failure throws the whole tree away** (ADR-0037). `LayoutEngine::reflow` returns `Result<(), LayoutAborted>`: a 10 s wall-clock deadline is polled on a stride (`layout::budget::checkpoint`, at taffy's `compute_box_layout` funnel and at every unbounded loop the crate owns), and a `catch_unwind` around the whole pass turns both a tripped deadline and a panic *inside* layout — taffy's `u16` grid guards — into that error instead of a dead page thread. There is no half-laid-out state to salvage, and that is exactly the unwind-safety argument: the box tree, the build snapshot and the success stamp are all discarded, so recovery is a full rebuild (the restyle set was already drained before the abort was possible). Adding a new unbounded loop in `layout` means adding a `checkpoint()`; adding one that can be reached from a `Drop` impl is a panic-during-unwind, i.e. an abort. `aborted_stamp` makes a retry at the same stamp fail fast, so a bad page burns one budget, not one per caller — and `set_budget` clears that gate when the limit *changes*, which is the only way a static document ever recovers from an abort. Upward the abort travels out-of-band — `Page::take_layout_abort()`, overwritten in full by every flush — because a geometry getter answers zeros (the `display: none` answer, never half a rectangle) and paint keeps the **previous** display list rather than caching a blank one. An artifact built from an aborted flush (screenshot, PDF, layout metrics, a CLI dump) must report the failure, not ship the empty result.
...
**The command port is a task source, and ordinary jobs obey the navigation guard.** A `PageJob` arriving mid-wait runs immediately only if `!in_job && !navigating && !state.parsing()`; otherwise it parks in `pending_jobs` and drains at the top of the loop, after the navigation drain (which invalidates every node id a job might hold) and before the page's own sources (a job is the driver's turn and must not be starved). **A suspended page is deliberately not excluded** (ADR-0034 D3): a pause for a debugger stops the page's own scheduling, not the protocol, and a driver sends its whole session setup before `Runtime.runIfWaitingForDebugger` — deferring that deadlocks the setup the pause exists to allow. So the suspended branch of the loop runs the driver's turn and nothing else: commands, non-main job queues, mutation records, binding payloads and deferred awaits. Anything reachable from a job that *waits* on the page's own work is the sharp edge here — `wait_until` returns instantly while suspended, so a loop around `settle` must test `cannot_progress()` or it spins. This is the same guard ADR-0022 gave script navigation, for the same reason: `await_subresources` and `await_pending_stylesheets` re-enter the loop from *inside* `load_document`, holding `RefCell` borrows on dom and style plus live parser handles, so a job that evaluated script there is a deterministic `BorrowMutError`, not a race. **`control` jobs are the exception and run at any wait point** — today `Page::request_close`, `Page::suspend`, `Page::resume` and `Page::is_suspended`, which is what keeps a page interruptible while it is blocked on a slow load. (The protocol reads `is_suspended` off an atomic mirrored on `PageHandle` instead, because its caller is a connection's event thread and even a control round trip can stall behind a page spinning in script.) `PageJob::control` is public, so the bar is a convention you must hold: `Cell`s and channels only. Anything that enters JS, borrows the DOM or flushes layout is a `BorrowMutError` waiting for the right timing. A dialog answer is the related case that is *not* a job at all — it travels on its own rendezvous channel, because a page parked in `run_dialog` services no ordinary job and an answer queued on the command port would sit behind the very block it is meant to release.

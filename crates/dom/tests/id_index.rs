...
fn parsed_document_is_indexed() {
    ...
    assert_eq!(
        tree.element_by_id(tree.document(), "a"),
        Some(find_element(&tree, "div"))
    );
    assert_eq!(
        tree.element_by_id(tree.document(), "b"),
        Some(find_element(&tree, "span"))
    );
    assert_eq!(tree.element_by_id(tree.document(), "missing"), None);
...
fn detached_element_is_indexed_only_while_connected() {
    ...
    assert_eq!(
        tree.element_by_id(tree.document(), "x"),
        None,
        "detached must not be indexed"
    );

    tree.append_child(body, div).unwrap();
    assert_eq!(tree.element_by_id(tree.document(), "x"), Some(div));

    tree.remove_child(body, div).unwrap();
    assert_eq!(tree.element_by_id(tree.document(), "x"), None);
}
...
fn descendants_of_an_inserted_subtree_are_indexed() {
    ...
    assert_eq!(tree.element_by_id(tree.document(), "inner"), None);

    tree.append_child(body, outer).unwrap();
    assert_eq!(tree.element_by_id(tree.document(), "inner"), Some(inner));
...
fn changing_and_removing_the_id_attribute_updates_the_index() {
    ...
    assert_eq!(tree.element_by_id(tree.document(), "old"), None);
    assert_eq!(tree.element_by_id(tree.document(), "new"), Some(div));

    tree.remove_attribute(div, &attr_name(local_name!("id")));
    assert_eq!(tree.element_by_id(tree.document(), "new"), None);
    assert_eq!(tree.id_names_in(tree.document()).count(), 0);
}

#[test]
fn moving_a_node_between_parents_keeps_it_findable() {
    let mut tree = parse("<div id='a'></div><div id='b'></div>");
    let a = tree.element_by_id(tree.document(), "a").unwrap();
    let b = tree.element_by_id(tree.document(), "b").unwrap();
    let child = new_div(&mut tree, "child");
    tree.append_child(a, child).unwrap();
    assert_eq!(tree.element_by_id(tree.document(), "child"), Some(child));
...
fn clone_is_not_indexed_until_inserted() {
    ...
    let body = body(&tree);
    let original = tree.element_by_id(tree.document(), "a").unwrap();
    ...
    assert_eq!(tree.element_by_id(tree.document(), "a"), Some(copy));
}
...
fn duplicate_ids_resolve_in_tree_order() {
    ...
    let body = body(&tree);
    let first = tree.element_by_id(tree.document(), "dup").unwrap();
    ...
    assert_eq!(tree.element_by_id(tree.document(), "dup"), Some(earlier));

    tree.remove_child(body, earlier).unwrap();
    assert_eq!(tree.element_by_id(tree.document(), "dup"), Some(first));
}
...
fn id_version_moves_on_index_changes_only() {
    ...
    assert_eq!(tree.element_by_id(tree.document(), "z"), None);
    let _ = detached;
}
